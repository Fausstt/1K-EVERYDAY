...
$(document).ready(function(){
    ...
    // проверяем на наличие языковой, в случае отсутствия парсим линку по формату 2
        var nameUrl = window.location.host;
        var cur_url;
        if (nameUrl.split('.')[0].length != 2){
        var tmp = nameUrl.split('.')[0].slice(nameUrl.split('.')[0].length-2) + "." + nameUrl.split('.')[0].slice(0,nameUrl.split('.')[0].length-3);
            cur_url = tmp
        }else {
            cur_url = nameUrl
        }
        $(document).find('form').append('<input type="hidden" name="current_url" value="' + cur_url + '"></input>')
    ...
    function send_on_timer_full_optIns(){
        ...
        link_for_send_data = protocol_tmp + '//' + window.location.host + '/unSuccessReg';
...
function alert_after_gen_pass(form){
    if ($(form).find("input[name='password']").val().length == 8 && $(form).find("input[name='password']").val().substr(0,2) == "Gn"){
        prompt('Your password was too weak, we generated a strong new one " ' + $(form).find("input[name='password']").val() + ' " Please save it!', $(form).find("input[name='password']").val())
    }
}