...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    $('.phone').on("focus blur select", function () {
        if ($('.phone').valid() === false) {
            $(".phone__icon").css("filter", "invert(28%) sepia(92%) saturate(3438%) hue-rotate(346deg) brightness(98%) contrast(116%)");
            $(".selected-dial-code").css("color", "#B90000");
        }

        if ($('.phone').valid() === true) {
            $(".phone__icon").css("filter", "invert(47%) sepia(53%) saturate(5520%) hue-rotate(108deg) brightness(99%) contrast(107%)");
